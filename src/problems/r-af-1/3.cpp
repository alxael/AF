...
int main()
{
    ...
    int nodeCount, vertexCount, increment, source, destination, maxFlow;
    in >> nodeCount >> vertexCount >> increment;
    ...
    int incrementedMaxFlow = 0;
    for(auto &vertex: adjacencyList[1]) {
        vertex.second += increment;
        incrementedMaxFlow = max(incrementedMaxFlow, edmondsKarp(nodeCount, 1, nodeCount, adjacencyList));
        vertex.second -= increment;
    }

    out << incrementedMaxFlow - maxFlow;
    return 0;
}